...
## 와이어프레임 (화면별 레이아웃 — 레이아웃의 진실 소스)
> BR-* 가 *동작*의 진실 소스라면, 여기는 *레이아웃*의 진실 소스. AI 구현 시 즉흥 레이아웃 금지.
> 화면당 ASCII 와이어프레임 1개. **design-system 컴포넌트 이름으로 표기**(Card/Button/EmptyState/Skeleton…).
> 4가지 UI 상태(로딩/정상/빈/에러)를 어디서 어떤 컴포넌트로 처리하는지 함께 표기 (FRONTEND_RULES).
...
│  · 항목/필드                   │
│ [버튼]  (Button primary)      │
│ 빈:EmptyState 로딩:Skeleton 오류:EmptyState(WifiOff) │
└──────────────────────────────┘
```
- 주요 인터랙션: [탭/스와이프/롱프레스 → 결과]
- 햅틱/애니메이션 포인트: (해당 시)
