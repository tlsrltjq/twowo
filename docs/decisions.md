...
## ADR-018: 1차 MVP 범위 축소 + 2차 분리 (범위 재정의)
- **결정**: "0~8단계 전부를 1차로 완주" 하던 계획을, **매일 여는 핵심 6개만 1차 MVP** 로 줄이고 나머지는 **TestFlight 실사용 검증 게이트 통과 후 2차**로 미룬다.
- **배경**: 기존 로드맵(0~8)은 MVP 가 아니라 완제품 스펙이었다. 0단계 상태에서 실험 기능 3종(투표/컨디션/빙고) + 실험실 + 30일 유예 Scheduled Function + 공개 출시(Apple 로그인/계정삭제 CF)까지 1차로 묶여, 완성 전에 동력이 꺼질 위험이 컸다. 차별성·retention(기념일)·수익 관점도 약했다.

### (1) 1차 MVP — "둘이 매일 여는 이유" 6개만
1. **인증 + 커플 연결** (stage-1) — **이메일 로그인만**. 구글/Apple 제외(구글 넣으면 Apple 로그인 의무 동반). `ensureCouple` 로 발급 전 커플 선생성(auth-couple BR-0).
2. **공유 캘린더(월간 1뷰) + 사진** (stage-2 축소) — 4뷰 중 📅 월간 + 🖼️ 사진 2뷰만. 운동/데이트 뷰는 2차. 사진은 라이브러리 선택 + 압축 + **EXIF 제거**(architecture).
3. **오늘의 컨디션** (stage-3b) — 그대로. retention 동력.
4. **홈 요약 + 기념일 디데이** (stage-4 축소) — 각 feature 합성 + **사용자 지정 기념일**(`couples.anniversaryDate`, home BR-2). 알림은 **로컬만**.
5. **단순 커플 해제** (stage-6 축소) — `status:'disconnected'` 전환 + 로그아웃 수준. 30일 유예/Scheduled Function 은 2차.
6. **TestFlight 실사용 게이트** (stage-7) — 둘이 2~4주 실제 사용. "정말 매일 쓰는가" 검증. **이 게이트 통과 전에는 2차 미진입.**

추가로 1차에 반드시 포함: 비밀번호 재설정, `expoPushToken` 필드 통일(아님: fcmToken), 필요한 복합 인덱스(`firestore.indexes.json`).

### (2) 2차 — 게이트 통과 후
- **둘다좋아 투표**(stage-3a), **데이트 빙고**(stage-3c) — 신선하나 retention 약함/금방 질림. 핵심 검증 후.
- **원격 푸시**(home BR-8) — **Cloud Function 경유만**(클라이언트 직접 발송 금지). Blaze 플랜 필요.
- **실험실 탭**(stage-5) — 기능 3개+ 일 때.
- **30일 유예 + Scheduled Function**(stage-6 전체) — 운영 복잡, 공개 출시 직전.
- **공개 출시**(stage-8) — 구글/Apple 로그인, 계정삭제 Cloud Function, 개인정보 처리방침. 별도 마일스톤.

### (3) 함께 고친 구현 버그 2건 (스펙 레벨)
- **트랜잭션 내 쿼리**: auth-couple `createInvite` 의 `tx.get(query(...))` 는 Firestore 미지원 → 트랜잭션 밖 `getDocs` + `writeBatch` 로 교체(auth-couple BR-3 + 쓰기 패턴).
- **클라이언트 직접 푸시**: home `sendPushToPartner` 가 상대 토큰을 읽어 직접 발사 → 토큰 노출/스팸. 서버(Cloud Function onWrite)에서만 발송하도록 변경(home BR-8). 1차에는 원격 푸시 자체 없음.

- **트레이드오프**: stage 번호는 **재번호하지 않는다**(교차 참조 다수 보존). 대신 "1차/2차" 분류를 ADR-018·HARNESS 로드맵·current.md 에 오버레이. 일부 stage 는 "축소 구현(1차) → 나머지(2차)" 로 쪼개진다.
...
## ADR-023: Firestore 스키마 마이그레이션 정책 (2026-06-12)
- **결정**: 운영 중 두 기기가 서로 다른 앱 버전을 실행할 수 있으므로, Firestore 스키마 변경은 아래 규칙을 따른다.
- **이유**: 한 명이 업데이트하고 상대방이 아직 구버전을 쓰는 상황에서 필드 삭제·의미 변경이 일어나면 구버전 앱이 크래시 또는 잘못된 데이터를 쓸 수 있다.

### 규칙
1. **필드는 추가만** — 기존 필드 삭제·이름 변경·타입 변경·의미 변경 금지. 새 의미가 필요하면 새 필드(`fieldV2` 등)를 추가하고 구 필드는 `optional`로 유지.
2. **삭제 3단계 절차** — 구 필드를 완전히 제거해야 할 경우:
   - 1단계: 신 필드 추가 + 신·구 필드 병행 쓰기 (배포)
   - 2단계: 양쪽 기기가 신 버전으로 업데이트됨을 확인 (TestFlight/배포 메트릭)
   - 3단계: 구 필드 읽기 코드 제거 + 신 필드만 쓰기 (다음 배포)
3. **optional 필드 기본값** — 새 필드는 TypeScript 타입에서 `?` 선택적으로 선언. 읽는 쪽은 반드시 `?? defaultValue`로 fallback 처리.
4. **열거형(enum) 확장만** — 기존 값 제거 금지. 값을 추가하는 것만 허용. 구버전이 모르는 값을 받아도 UI가 graceful degradation 해야 함.
5. **인덱스 추가는 먼저** — 새 쿼리가 필요한 인덱스는 앱 배포 전에 `firebase deploy --only firestore:indexes`로 선배포. 앱 코드와 동시 배포 시 인덱스 빌드 지연으로 쿼리 실패 가능.
...
- **결정**: 아래 항목은 사용자가 직접 진행 의사를 밝힐 때까지 대기. 그 전까지는 시뮬레이터에서 검증 가능한 기능 개발에 집중.
  ...
  - Firebase Storage Blaze 플랜 → Storage 보안 규칙 배포 · 사진 업로드 활성화
  - Cloud Function / Scheduled Function → 원격 푸시(BR-8) · 30일 유예 커플 해제
