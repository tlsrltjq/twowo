...
> 디테일: `docs/frontend.md`(패턴/코드) · `docs/design-system.md`(토큰/컴포넌트) · `docs/decisions.md`(ADR).
...
- Zustand store **4개만** (`auth`/`couple`/`ui`/`feature`). 서버 상태 store 저장 금지. 새 store 추가 전 사용자 확인.
  → 훅 코드·store 책임/영속화 표: **`frontend.md` 데이터 페칭 훅 / Zustand**
...
- 애니메이션: 마이크로(≤200ms) = Reanimated, 큰 축하 = Lottie 1회. 입력 막는 애니메이션 금지.
  → 낙관적/햅틱/애니메이션 상세 표: **`frontend.md` · `design-system.md`**
...
- 로그: 개발은 `__DEV__` 가드 안에서만 `console.*`, 프로덕션은 babel-plugin 으로 제거.
  → 권한 rationale 문구 표·라우팅 가드 패턴: **`frontend.md`**
